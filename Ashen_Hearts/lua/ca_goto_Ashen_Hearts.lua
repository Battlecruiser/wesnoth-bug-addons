...
local cfg = {
    ca_score=110000,
    filter_location = { x = '37,32,27,20,30,27,35,16', y = '71,67,70,58,58,64,53,52' },
    enemy_side = 1
}
 
local ca_goto_AH = {}
 
function ca_goto_AH:evaluation(ai, dummy_cfg, self)
    ...
    for _,unit in ipairs(units) do
        ...
        for _,enemy in ipairs(enemies) do
            local dist_enemy = H.distance_between(unit.x, unit.y, enemy.x, enemy.y)
        ...
        for _,loc in ipairs(locs) do
            ...
            local dist_goal_enemy = H.distance_between(closest_enemy.x, closest_enemy.y, loc[1], loc[2])
    ...
    self.data.GO_unit, self.data.GO_dst, self.data.GO_do_full_move = best_unit, best_hex, do_full_move
 
    return cfg.ca_score
end
 
function ca_goto_AH:execution(ai, dummy_cfg, self)
    if self.data.do_full_move then
        --print('  best unit full move:', self.data.GO_unit.id, self.data.GO_unit.x, self.data.GO_unit.y, self.data.GO_dst[1], self.data.GO_dst[2])
        AH.checked_move_full(ai, self.data.GO_unit, self.data.GO_dst[1], self.data.GO_dst[2])
    else
        --print('  best unit partial move:', self.data.GO_unit.id, self.data.GO_unit.x, self.data.GO_unit.y, self.data.GO_dst[1], self.data.GO_dst[2])
        AH.checked_move(ai, self.data.GO_unit, self.data.GO_dst[1], self.data.GO_dst[2])
    end
 
    self.data.GO_unit, self.data.GO_dst, self.data.GO_do_full_move = nil, nil, nil
end
