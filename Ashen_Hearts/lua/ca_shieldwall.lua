...
local function get_best_formation(units_row1, units_row2, y0, cfg)
    ...
    local previous_min_x = wesnoth.get_variable('AI_shieldwall_S' .. wesnoth.current.side .. '_min_x') or cfg.start_min_x
    ...
    for x0 = 1,width-wall_width+1 do -- xxx hard-coded behavior (scanning through x only)
        ...
        for dx = 0, wall_width-1 do
            ...
            local movecost = wesnoth.unit_movement_cost(test_unit_row1, wesnoth.get_terrain(x1,y1))
            ...
            local unit_in_way = wesnoth.get_unit(x1, y1)
            if unit_in_way then
        ...
        if terrain_possible then
            for i = 1, wall_width do
                ...
                local movecost = wesnoth.unit_movement_cost(test_unit_row2, wesnoth.get_terrain(x1,y1))
...
function ca_shieldwall:evaluation(ai, cfg, self)
    units_row1 = wesnoth.get_units {
...
function ca_shieldwall:execution(ai, _, self)
    ...
    wesnoth.set_variable('AI_shieldwall_S' .. wesnoth.current.side .. '_min_x' , min_x)
    wesnoth.set_variable('AI_shieldwall_S' .. wesnoth.current.side .. '_min_y' , min_y)

    self.data.SW_moves = nil
end
